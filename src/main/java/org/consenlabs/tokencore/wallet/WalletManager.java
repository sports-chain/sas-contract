...
public class WalletManager {
	...
	public static void removeWallet(String id, String password) throws Exception {
		Wallet wallet = mustFindWalletById(id);
	...
	public static Wallet importWalletFromKeystore(Metadata metadata, String keystoreContent, String password, boolean overwrite) throws Exception {
	...
	public static Wallet importWalletFromPrivateKey(Metadata metadata, String prvKeyHex, String password, boolean overwrite) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static Wallet importWalletFromMnemonic(Metadata metadata, @Nullable String accountName, String mnemonic, String path, String password, boolean overwrite) throws Exception {
		if (metadata.getSource() == null) {
	...
	public static Wallet importWalletFromMnemonic(Metadata metadata, String mnemonic, String path, String password, boolean overwrite) throws Exception {
		return importWalletFromMnemonic(metadata, null, mnemonic, path, password, overwrite);
	}
	...
	public static Wallet setAccountName(String id, String accountName) throws Exception {
		Wallet wallet = mustFindWalletById(id);
	...
	static File generateWalletFile(String code) {
		return new File(getDefaultKeyDirectory(), code + ".json");
	}

	static File getDefaultKeyDirectory() {
		File directory = null;
		if (1001 == Constant.os) {
			directory = new File("D:/Documents/wallet");
		} else if (1002 == Constant.os) {
			directory = new File("/home/wallet");
		}
		if (!directory.exists()) {
			directory.mkdirs();
		}
	...
	private static Wallet persistWallet(IMTKeystore keystore, boolean overwrite) throws Exception {
		Wallet wallet = flushWallet(keystore, overwrite);
		Identity.getCurrentIdentity().addWallet(wallet);
		return wallet;
	}
		...
		for (File file : directory.listFiles()) {
			if (!file.getName().startsWith("identity")) {
				try {
					...
					log.info("Can't loaded " + file.getName() + " file", e);
				}
			}
		}
	}
