...
public class BitcoinTransaction implements TransactionSigner {
	...
	public BitcoinTransaction(String to, int changeIdx, long amount, long fee, List<UTXO> outputs) {
		this.to = to;
		this.amount = amount;
		this.fee = fee;
	...
	public String toString() {
		return "BitcoinTransaction{" + "to='" + to + '\'' + ", amount=" + amount + ", outputs=" + outputs + ", memo='" + memo + '\'' + ", fee=" + fee + ", changeIdx=" + changeIdx + '}';
	}
	...
	public TxSignResult signTransaction(int chain_id, String password, Wallet wallet) {
	...
	public TxSignResult signUsdtTransaction(int chain_id, String password, Wallet wallet) {
		collectPrvKeysAndAddress(Metadata.NONE, password, wallet);
	...
	public TxSignResult signUsdtCollectTransaction(int chain_id, String password, Wallet wallet, Wallet feeProviderWallet, List<UTXO> feeProviderUtxos) {
...
//      stream.write(changeScriptPubKey);
			...
			UnsafeByteArrayOutputStream[] serialStreams = new UnsafeByteArrayOutputStream[] { new UnsafeByteArrayOutputStream(), new UnsafeByteArrayOutputStream() };
...
//			DeterministicKey changeKey = HDKeyDerivation.deriveChildKey(xprvKey, ChildNumber.ONE);
			HDKeyDerivation.deriveChildKey(xprvKey, ChildNumber.ONE);
			// 将找零指向自己地址
			changeAddress = Address.fromBase58(network, wallet.getAddress());
